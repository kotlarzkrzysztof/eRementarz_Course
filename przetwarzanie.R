...
dane %>% 
  filter((subpanel %in% c('NZ1', 'NZ2')) & budget > 1000000 & type=='SONATA') %>% 
  dim()
...
dane %>% filter(type == 'SONATINA', panel =='ST') %>% group_by(subpanel) %>% summarise(srednia = mean(budget))

# Zadanie 3
# Wylicz ile grantów PRELUDIUM o budżecie powyżej 100 tys. zł zostało przyznanych w poszczególnych panelach 

dane %>% filter(type == 'PRELUDIUM' & budget > 100000) %>% 
  group_by(panel) %>% 
  summarise(count = n())
